...
void learnGMMfromSamples(const vector<StructureTensor> &samples, Mat& model, int nr_gaussians = 5)
{
    ...
    vector<StructureTensor> tmp_centers;

    kmeans( samples, componentsCount,
            TermCriteria( CV_TERMCRIT_ITER, kMeansItCount, 0.0), 4, labels, tmp_centers);
...
int main( int argc, char** argv )
{
    ...
    for(vector<string>::iterator filename = input_images.begin(); filename != input_images.end(); ++filename)    
    {
        ...
        StructureTensorImage stimage(image);
